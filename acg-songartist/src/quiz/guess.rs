...
impl quiz::SongArtistQuiz {
    pub fn handle_guess(&self, guess: &str, time: f32) -> Option<GuessResult> {
        ...
        // check song name
        let song_guess = check_song_name(
            &guess_norm,
            &guess,
            guess_info,
            edit,
        ...
        for (best_artist_guess, artist, display) in itertools::izip!(best_artists, artists, &mut display_artists) {
            if let Guess::Incorrect(ref old, percent) = best_artist_guess {
                ...
                if guess_artist > settings.answer_threshold {
                    *best_artist_guess = Guess::Correct(edited.unwrap_or_else(|| guess.to_string()), time);
                    *display = true;
                    num_correct_artists += 1;
                } else if guess_artist > *percent {
                    *best_artist_guess = Guess::Incorrect(edited.unwrap_or_else(|| guess.to_string()), guess_artist);
