...
use deadpool_postgres::{
    BuildError, Manager, ManagerConfig, Object, Pool as PostgresPool, RecyclingMethod,
};
use std::sync::Arc;
...
impl Database {
    pub fn new<S: AsRef<str>>(connection_string: S) -> Self {
        let inner = match connection_string.as_ref().parse::<PostgresConfig>() {
            ...
            Ok(pg_config) => {
                ...
                match PostgresPool::builder(manager).build() {
                    ...
                    Err(BuildError::NoRuntimeSpecified(e)) => {
                        DatabaseInner::NoRuntimeSpecified(e.into())
                    }
        ...
        match inner {
            ...
            | DatabaseInner::Error(ref s) => {
                log::error!("failed to init database connection: {}", s)
            }
        }
