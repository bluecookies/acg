...
#[cfg(feature = "sappho")]
fn with_bot(bot: sappho_bot::Bot) -> impl Filter<Extract = (sappho_bot::Bot,), Error = std::convert::Infallible> + Clone {
...
#[cfg(feature = "sappho")]
pub fn connect_sa_bot(bot: sappho_bot::Bot) -> impl Filter<Extract = (impl Reply,), Error = Rejection> + Clone {
        ...
        .then(|bot: sappho_bot::Bot, form: Form| async move {
            ...
            match bot.connect(cookie).await {
                Ok(c) => {
                    ...
                    warp::redirect::see_other(warp::http::Uri::from_static("/status")).into_response()
                },
                Err(e) => {
